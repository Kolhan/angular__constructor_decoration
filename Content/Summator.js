var squareRooms = {
    ...
    sumPoint: [
        ...
        { id: 21, name: "Подсветка",                              price: 0, count: 0, sum: 0, pm: page3_model4},
        { id: 22, name: "Шторка",                                 price: 0, count: 0, sum: 0, pm: page3_model4},
        { id: 23, name: "Аксессуары",                             price: 0, count: 0, sum: 0, pm: page3_model4},
        { id: 24, name: "Стиральная машинка",                     price: 0, count: 0, sum: 0, pm: page3_model4},
        { id: 25, name: "Шкаф",                                   price: 0, count: 0, sum: 0, pm: page3_model4},
    ],
...
NextProjectApp.controller("SummatorController", function ($scope) {
    ...
    $scope.FillSquare = function (summer, p1m1) {
        ...
        if  (summer.counter[17].val>0 && summer.counter[17].val<=34) {summer.counter[18].val=40}; 
        if  (summer.counter[17].val>34 && summer.counter[17].val<=60) {summer.counter[18].val=55};
        if  (summer.counter[17].val>60 && summer.counter[17].val<=80) {summer.counter[18].val=65};
        if  (summer.counter[17].val>80 && summer.counter[17].val<=110) {summer.counter[18].val=75};
        if  (summer.counter[17].val>110 && summer.counter[17].val<=130) {summer.counter[18].val=85};
        if  (summer.counter[17].val>130) {summer.counter[18].val=100};
        ...
        for (var g = 0; g <= summer.sumPoint.length-1; g++) {            
            ...
            //Пол
            if  (g == 5) { 
                //setSumTabs(g, summer.counter[3].val) 
                panel = summer.sumPoint[g].pm;                
                resultCount = summer.counter[3].val;
                //indexSumPoint = g;
                //for (var j = 0; j <= summer.sumPoint[indexSumPoint].pm.tabs.length-1; j++) {
                //    summer.sumPoint[indexSumPoint].pm.tabs[j].sum = resultCount * summer.sumPoint[indexSumPoint].pm.function_tabs[j].price_min;
                //}

                resultPrice = panel.function_tabs[panel.selectFunctionTabIndex].price_min;      
                resultSum = roundPlus(resultPrice*resultCount,2); 
            };
            //Двери
            ...
            if  (g == 19) { 
                ...
                /*
                    стоимость полов = (Sполов+Sлоджий)*price
                    стоимость керамогранитных полов = (кухня+прихожая+лоджии)*(стоимость керамгранита-стоимость пола)
                */
                var squareMinus = (summer.counter[3].val+summer.counter[5].val);
                var priceMinus = (squareMinus*summer.sumPoint[5].pm.tabs[summer.sumPoint[5].pm.selectTabIndex].price>0)?squareMinus*summer.sumPoint[5].pm.function_tabs[summer.sumPoint[5].pm.selectFunctionTabIndex].price_min/squareMinus:0;
                var indexSumPoint = g;                
                resultCount = _count;                
                ...
                resultPrice = summer.sumPoint[indexSumPoint].pm.tabs[summer.sumPoint[indexSumPoint].pm.selectTabIndex].price-priceMinus;                 
                resultSum = roundPlus(resultPrice*resultCount,2); 
            ...
            //Подсветка
            if  (g == 21) { fillSumPoint(1, summer.sumPoint[g].pm.footerCheckOptions[1].checked ? summer.sumPoint[g].pm.footerCheckOptions[1].price : 0) };
            //Шторка
            if  (g == 22) { fillSumPoint(1, summer.sumPoint[g].pm.footerCheckOptions[2].checked ? summer.sumPoint[g].pm.footerCheckOptions[2].price : 0) };
            //Аксессуары
            if  (g == 23) { fillSumPoint(1, summer.sumPoint[g].pm.footerCheckOptions[3].checked ? summer.sumPoint[g].pm.footerCheckOptions[3].price : 0) };
            //Стиральная машинка
            if  (g == 24) { fillSumPoint(1, summer.sumPoint[g].pm.footerCheckOptions[4].checked ? summer.sumPoint[g].pm.footerCheckOptions[4].price : 0) };
            //Шкаф
            if  (g == 25) { fillSumPoint(1, summer.sumPoint[g].pm.footerCheckOptions[5].checked ? summer.sumPoint[g].pm.footerCheckOptions[5].price : 0) };
        ...
        for (var g = 0; g <= summer.sumOption.length-1; g++) {            
            ...
            resultSum = roundPlus(resultCount * resultPrice,0);
            
            //В опции "Теплый пол в санузле" ее стоимость умножаем на метраж всей площади
            if(g==2){ resultCount = summer.counter[17].val; }
            //В опции "Теплый пол на кухне" ее стоимость умножаем на метраж всей площади
            if(g==3){ resultCount = summer.counter[17].val; }
